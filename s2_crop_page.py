...
def read_json(file):
    with open(file) as f:
        ...
        for i in range(13759):
            if (128 <= i <= 255) or (0 <= i < 32): # 128 - 255: 'UNICODE' = '     '; 0 - 31: unable to print
                unicode[i] = '123'
            else:
...
def scaleAdjustment(word_img, adjustCentroid=True):
    ...
    # cv2.imshow('binary word img', binary_word_img)
    # key = cv2.waitKey(0)
    ...
    for j in range(10):
        ...
        for k in range(10):
            ...
            if index > len(unicode):
                break
            if unicode[index - 1] == '100': # skip (ignore character)
                continue
